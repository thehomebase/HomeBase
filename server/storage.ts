    ...
    try {
      const result = await db.execute(sql`
        INSERT INTO transactions (
          ...
          ${insertTransaction.optionPeriodExpiration || null},
      ...
      return {
        ...
        optionPeriodExpiration: row.option_period_expiration ? Number(row.option_period_expiration) : null,
  ...
  async getTransaction(id: number): Promise<Transaction | undefined> {
    try {
      ...
      const transaction: Transaction = {
        ...
        optionPeriodExpiration: row.optionPeriodExpiration ? Number(row.optionPeriodExpiration) : null,
    ...
    try {
      const result = await db.execute(sql`
        SELECT 
          ...
          t.option_period_expiration::integer as "optionPeriodExpiration",
