    ...
    try {
      ...
      const result = await db.execute(sql`
        SELECT 
          ...
          buyer_agent_compensation as "buyerAgentCompensation",
          listing_agent_commission as "listingAgentCommission",
          home_warranty as "homeWarranty",
      ...
      const transaction: Transaction = {
        ...
        participants: Array.isArray(row.participants) ? row.participants : [],
        contractPrice: row.contractPrice != null ? Number(row.contractPrice) : null,
        optionPeriodExpiration: row.optionPeriodExpiration ? new Date(row.optionPeriodExpiration) : null,
        optionFee: row.optionFee != null ? Number(row.optionFee) : null,
        earnestMoney: row.earnestMoney != null ? Number(row.earnestMoney) : null,
        downPayment: row.downPayment != null ? Number(row.downPayment) : null,
        sellerConcessions: row.sellerConcessions != null ? Number(row.sellerConcessions) : null,
        buyerAgentCompensation: (row as any).buyerAgentCompensation != null ? Number((row as any).buyerAgentCompensation) : null,
        listingAgentCommission: (row as any).listingAgentCommission != null ? Number((row as any).listingAgentCommission) : null,
        homeWarranty: (row as any).homeWarranty != null ? Number((row as any).homeWarranty) : null,
  ...
  async updateTransaction(id: number, data: Partial<Transaction>): Promise<Transaction> {
    try {
      ...
      return {
        ...
        optionPeriod: row.option_period != null ? Number(row.option_period) : null,
        optionFee: row.option_fee != null ? Number(row.option_fee) : null,
        earnestMoney: row.earnest_money != null ? Number(row.earnest_money) : null,
        downPayment: row.down_payment != null ? Number(row.down_payment) : null,
        sellerConcessions: row.seller_concessions != null ? Number(row.seller_concessions) : null,
        buyerAgentCompensation: row.buyer_agent_compensation != null ? Number(row.buyer_agent_compensation) : null,
        listingAgentCommission: row.listing_agent_commission != null ? Number(row.listing_agent_commission) : null,
        homeWarranty: row.home_warranty != null ? Number(row.home_warranty) : null,
