...
- **Admin Dashboard**: Redesigned with Recharts charts (users-by-role growth/cumulative line chart with toggle, users-by-role donut chart), stat cards with trend badges, recent signups feed, and tabbed management (Users, Verifications, Reports, Ads, Financial, Messages, Audit Log). Financial tab shows Stripe subscription/revenue stats (active subs, total revenue, Stripe customers, ad revenue), monthly revenue bar chart, subscriptions-by-status donut chart, and recent invoices table. User table has pagination (10/page). Admin messages system: `admin_messages` DB table, `POST /api/contact-admin` for users, `GET /api/admin/messages` and `POST /api/admin/messages/:id/reply` for admin. Users table supports search and role filtering.
