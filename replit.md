...
- **Communication Metrics Dashboard**: Toggleable stats panel on Messages page showing SMS sent (today/week/month), emails sent, private messages sent, unique contacts/recipients, and all-time totals. API: `GET /api/communications/metrics`. Queries `communications`, `email_tracking`, and `private_messages` tables with date-range filters. Available for agent, vendor, and lender roles. Key files: `server/storage.ts` (`getCommunicationMetrics`), `server/routes.ts`, `client/src/pages/messages-page.tsx`.
