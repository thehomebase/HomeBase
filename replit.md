...
- **Admin Dashboard**: Redesigned with Recharts charts (user/tx growth area chart, users-by-role pie chart, transactions-by-status horizontal bar chart), stat cards with trend badges, recent signups feed, and tabbed management (Users, Verifications, Reports, Ads, Messages, Audit Log). Admin messages system: `admin_messages` DB table, `POST /api/contact-admin` for users, `GET /api/admin/messages` and `POST /api/admin/messages/:id/reply` for admin. Users table supports search and role filtering.
