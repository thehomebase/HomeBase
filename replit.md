...
- **Profile Pages** (`/profile`, `/profile/:id`): Agent/broker profile pages with styled profile photos (gray background), brokerage name overlay, bio, license info, verification badges, contact details. Photo upload endpoint (`POST /api/profile/photo`) with sharp image processing. Edit own profile dialog. Public agent directory (`GET /api/agents`).
