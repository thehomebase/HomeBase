...
- **Team Access / Authorized Users**: Agents/brokers can add other agents/brokers as authorized users on their account for transaction coordination. Features: invite by email, accept/decline invitations, configurable permission levels (View Only or Full Access), account switcher dropdown in sidebar to toggle between own account and authorized accounts, `verifyTransactionAccess()` extended to grant authorized users access to owner's transactions. DB: `authorized_users` table with `owner_id`, `authorized_user_id`, `permission_level`, `status`. Settings page "Team Access" tab. Routes: `/api/authorized-users/*`. Transactions route supports `?actingAs=userId` query parameter.
- **Client Portal**: Redesigned mobile-first "My Transaction" page with package-tracking-style stage tracker (animated progress circles with icons), large closing countdown timer with urgency coloring, "Action Required" cards for pending signatures/deadlines/checklists, key dates grid with live countdowns, interactive document hub with "Sign Now" buttons, vertical connected-dot timeline with color-coded status indicators, collapsible financial details, "Your Team" section (agent photo/contact + transaction contacts with call/email/message buttons), and bottom quick-action bar. **Multi-transaction support**: clients with multiple transactions (buy + sell) see all transaction cards on dashboard with buy/sell indicators and per-transaction pending doc counts; transaction page has dropdown switcher to switch between transactions. Backend: `/api/client/my-transactions` lists all client transactions; `/api/client/my-transaction?transactionId=X` loads a specific transaction. `secondary_client_id` column on transactions links secondary clients. Authorization hardened with participant/client/secondary-client ownership verification.
...
- **Admin Dashboard**: Redesigned with Recharts charts (users-by-role growth/cumulative line chart with toggle, users-by-role donut chart), stat cards with trend badges, recent signups feed, and tabbed management (Users, Verifications, Reports, Ads, Financial, Leads, Geographic, Messages, Audit Log). Financial tab: MRR, active subs, ad revenue, lead gen revenue (by agent/lender/vendor), monthly revenue bar chart, subscriptions-by-status donut, recent invoices. Leads tab: conversion rate, avg response time, leads by source/status donut charts, top lead zip codes. Geographic tab: transactions by state donut, most active transaction zip codes, most claimed lead gen zip codes. API Usage tab: RentCast usage gauge with limits, Twilio SMS and Gmail daily bar charts with month-over-month trends, SignNow/DocuSign e-signature activity charts with action breakdowns, Stripe events, webhooks configured, email open rates. User table has pagination (10/page). Admin messages system: `admin_messages` DB table, `POST /api/contact-admin` for users, `GET /api/admin/messages` and `POST /api/admin/messages/:id/reply` for admin. Users table supports search and role filtering.
