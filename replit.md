...
- **Listing Video Creator**: AI-powered tool for agents/brokers to transform property photos into cinematic walkthrough-style videos. Gemini Flash vision analyzes each photo to determine room type, optimal motion path (pan, zoom), caption, and walkthrough ordering. Browser-based Canvas renderer with Ken Burns effects, crossfade transitions, text overlays, agent branding, multiple aspect ratios (landscape, portrait/Reels, square), and WebM export via MediaRecorder API. **3D Depth Parallax**: GPU-powered depth estimation via **fal.ai Depth Anything V2** API (`FAL_KEY` secret required); falls back to browser-based WASM model if API key not configured. Multi-layer parallax rendering via Three.js: depth map segments photo into 8 layers with edge dilation, places each as a textured plane at different Z-depths, perspective camera creates natural parallax. Files: `client/src/lib/depth-estimator.ts`, `client/src/lib/three-renderer.ts`. Server endpoint: `POST /api/listing-videos/generate-depth`. Table: `listing_videos`.
